...
import { AuditLog, Campaign, GHLSyncLog, Lead, Prisma } from "@prisma/client";
...
import { GhlAutoSyncSettings } from "@/lib/ghl/ghlAutoSyncSettings";
...
export function GhlSyncClient({
  ...
  previewLogs: GhlLog[];
  campaigns: Campaign[];
  autoSyncSettings: GhlAutoSyncSettings;
  autoSyncAuditLogs: AuditLog[];
}) {
  ...
  const autoSyncSkipped = autoSyncAuditLogs.filter((log) => log.action === "Auto-sync skipped");
  const lastAttemptByLead = useMemo(() => {
    const map = new Map<string, GhlLog>();
    for (const log of previewLogs) {
      if (!map.has(log.leadId)) map.set(log.leadId, log);
    }
    return map;
  }, [previewLogs]);
    ...
    <div className="space-y-6">
      <div className="flex flex-col justify-between gap-4 xl:flex-row xl:items-center">
        ...
        <div className="flex flex-wrap gap-2">
          <Badge tone={config.isConfigured ? "green" : "gold"}>{config.mode}</Badge>
          <Badge tone="black">{config.isConfigured ? "Manual live sync available" : "JSON preview only"}</Badge>
      ...
      <Card>
        <CardHeader>
          <div>
            ...
            <p className="text-sm text-neutral-600">Reply-triggered GHL sync is optional and off by default.</p>
          </div>
          <div className="flex flex-wrap gap-2">
            <Badge tone={autoSyncSettings.autoSyncInterestedLeadsToGhl ? "green" : "gold"}>
              Interested auto-sync {autoSyncSettings.autoSyncInterestedLeadsToGhl ? "enabled" : "disabled"}
            </Badge>
            <Badge tone={autoSyncSettings.autoSyncBookedLeadsToGhl ? "green" : "gold"}>
              Booked auto-sync {autoSyncSettings.autoSyncBookedLeadsToGhl ? "enabled" : "disabled"}
            </Badge>
          </div>
        </CardHeader>
        <div className="grid gap-3 md:grid-cols-4">
          <div className="rounded-md border border-black/10 bg-neutral-50 p-3 text-sm">
            <div className="text-xs uppercase text-neutral-500">Leads waiting for GHL</div>
            <div className="mt-1 text-2xl font-semibold">{readyLeads.filter((lead) => lead.ghlSyncStatus === "Ready for GHL").length}</div>
          </div>
          <div className="rounded-md border border-black/10 bg-neutral-50 p-3 text-sm">
            <div className="text-xs uppercase text-neutral-500">Auto-sync skipped logs</div>
            <div className="mt-1 text-2xl font-semibold">{autoSyncSkipped.length}</div>
          </div>
          <div className="rounded-md border border-black/10 bg-neutral-50 p-3 text-sm">
            <div className="text-xs uppercase text-neutral-500">Last sync attempt</div>
            <div className="mt-1 font-medium">{formatDate(previewLogs[0]?.createdAt)}</div>
          </div>
          <div className="rounded-md border border-black/10 bg-neutral-50 p-3 text-sm">
            <div className="text-xs uppercase text-neutral-500">Last sync error</div>
            <div className="mt-1 line-clamp-2 font-medium text-red-700">{failedLogs[0]?.error ?? "None"}</div>
          </div>
        </div>
        {autoSyncAuditLogs.length > 0 && (
          <div className="mt-4 space-y-2">
            {autoSyncAuditLogs.slice(0, 4).map((log) => (
              <div key={log.id} className="rounded-md border border-black/10 bg-white p-3 text-sm">
                <div className="font-medium">{log.action}</div>
                <div className="mt-1 text-neutral-600">{log.reason ?? "No reason recorded."}</div>
      ...
      <Card>
        ...
        <LeadTable
          leads={filteredReady}
          onPreview={previewLead}
          isPending={isPending}
          selected={selected}
          setSelected={setSelected}
          lastAttemptByLead={lastAttemptByLead}
        />
      </Card>
...
function LeadTable({
  ...
  lastAttemptByLead: Map<string, GhlLog>;
}) {
          ...
          <tbody className="divide-y divide-black/10">
            {leads.map((lead) => (
              <tr key={lead.id} className="align-top">
                ...
                <td className="px-3 py-3">{formatDate(lastAttemptByLead.get(lead.id)?.createdAt)}</td>
                <td className="px-3 py-3">
                  <div className="flex flex-wrap gap-2">
