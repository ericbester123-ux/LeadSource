...
const LIVE_CAMPAIGN_SENDING_ENABLED = false;

function getProvider() {
  if (isSimulationMode() || !LIVE_CAMPAIGN_SENDING_ENABLED) return simulatedEmailProvider;
...
export async function processDueQueue() {
  ...
  const liveSafety = isSimulationMode() || !LIVE_CAMPAIGN_SENDING_ENABLED ? { ok: true, reasons: [] } : await checkLiveSendingSafety();
  ...
  for (const item of dueItems) {
    ...
    if (item.scheduledFor && !isWithinSendingWindow(item.scheduledFor, windowStart, windowEnd)) {
    ...
    if (result.ok) {
      ...
      await logSendAttempt({
        ...
        status: provider.name === "simulation" ? "Simulated" : "Sent",
        providerMessageId: result.providerMessageId,
        metadata: result.metadata,
        sentAt
      });
      await writeAudit(provider.name === "simulation" ? "Email sent simulated" : "Email sent", "SendingQueueItem", item.id, provider.name === "simulation" ? "Queue item processed in simulation mode; live campaign sending disabled." : "Queue item sent by provider", {
        provider: result.provider,
        recipientEmail: item.recipientEmail
      });
      if (provider.name === "simulation") summary.simulated += 1;
      else summary.sent += 1;
      continue;
    }
