...
export async function checkQueueItemSafety(item: SendingQueueItemWithInclude): Promise<SafetyResult> {
  ...
  if (!lead.email || !item.recipientEmail) reasons.push("Recipient email is missing.");
  ...
  if (lead.country === "United States" && !rendered.hasBusinessAddress) reasons.push("Business address is missing for US outreach.");

  const suppressed = item.recipientEmail ? await prisma.suppressionListEntry.findUnique({ where: { email: item.recipientEmail } }) : null;
