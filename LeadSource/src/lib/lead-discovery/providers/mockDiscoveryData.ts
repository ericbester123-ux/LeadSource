...
export function getMockDiscoveryLeads(criteria: DiscoveryCriteria, sourcePlatform: string): DiscoveredLead[] {
  ...
  return Array.from({ length: max }).map((_, index) => {
    ...
    return {
      ...
      googlePlaceId: sourcePlatform.toLowerCase().includes("google") ? `mock-place-${slug(companyName)}-${suffix}` : undefined,
      googleRating: sourcePlatform.toLowerCase().includes("google") ? 4.1 + (index % 8) / 10 : undefined,
      googleReviewsCount: sourcePlatform.toLowerCase().includes("google") ? 12 + index * 7 : undefined,
      leadSourceNotes: `${sourcePlatform} mock result generated safely without scraping.`
    };
  });
}
