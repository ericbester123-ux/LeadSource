...
async function main() {
  ...
  const campaign = await prisma.campaign.upsert({
    ...
    update: {
      syncInterestedLeadsToGhl: false,
      syncBookedLeadsToGhl: false
    },
    create: {
  ...
  for (const [firstName, lastName, email, companyName, city, country, score, qualityLabel] of leads) {
    const lead = await prisma.lead.upsert({
      ...
      create: {
        ...
        status: Number(score) >= 85 ? "Ready for Review" : "Scored",
    ...
    await prisma.complianceCheck.upsert({
      ...
      create: {
        leadId: lead.id,
        status: Number(score) >= 85 ? "Passed" : "Pending Review",
  ...
  await prisma.settings.upsert({
    where: { key: "ghl_auto_sync_settings" },
    update: {
