...
export function SendingQueueClient({
  ...
  const [generationState, generateQueueAction, generationPending] = useActionState(generateQueueFromApprovedEnrollments, emptyQueueGenerationState);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-2">
        <form action={generateQueueAction}><Button disabled={generationPending}><RefreshCw className="h-4 w-4" /> Generate queue</Button></form>
      ...
      {generationState.ran && (
        <Card className={generationState.created ? "border-emerald-200 bg-emerald-50" : "border-gold-200 bg-gold-50"}>
          <CardHeader>
            <div>
              <h3 className="text-lg font-semibold">Last Generation Summary</h3>
              <p className="text-sm text-neutral-700">{generationState.message}</p>
            </div>
            <Badge tone={generationState.created ? "green" : "gold"}>{generationState.created} created</Badge>
          </CardHeader>
          <div className="grid gap-3 md:grid-cols-4 xl:grid-cols-7">
            {[
              ["Enrollments checked", generationState.enrollmentsChecked],
              ["Queue items created", generationState.created],
              ["Skipped / blocked", generationState.skipped],
              ["Already queued", generationState.alreadyQueuedSkipped],
              ["Missing email", generationState.missingEmail],
              ["Invalid email", generationState.invalidEmail],
              ["Verification", generationState.verificationStatus],
              ["Compliance", generationState.complianceStatus],
              ["Suppression", generationState.suppression],
              ["Do-not-contact", generationState.doNotContact],
              ["Unsubscribed", generationState.unsubscribed],
              ["Bounced", generationState.bounced],
              ["Low score", generationState.lowScore],
              ["Missing step/template", generationState.missingTemplate],
              ["Stop rules", generationState.stopRule],
              ["Other errors", generationState.otherErrors]
            ].map(([label, value]) => (
              <div key={String(label)} className="rounded-md border border-black/10 bg-white/80 p-3">
                <div className="text-xs text-neutral-500">{label}</div>
                <div className="mt-1 text-xl font-semibold">{value}</div>
              </div>
            ))}
          </div>
          {generationState.blockedLeads.length > 0 && (
            <details className="mt-4">
              <summary className="cursor-pointer text-sm font-semibold text-ink">View blocked leads</summary>
              <TableWrap className="mt-3">
                <div className="max-h-[340px] overflow-auto">
                  <Table className="min-w-[1000px]">
                    <thead className="sticky top-0 bg-neutral-50 text-xs uppercase text-neutral-500">
                      <tr><th className="px-3 py-3">Lead</th><th className="px-3 py-3">Campaign</th><th className="px-3 py-3">Step</th><th className="px-3 py-3">Category</th><th className="px-3 py-3">Reason</th></tr>
                    </thead>
                    <tbody className="divide-y divide-black/10 bg-white">
                      {generationState.blockedLeads.map((lead, index) => (
                        <tr key={`${lead.leadId}-${lead.category}-${index}`} className="align-top">
                          <td className="px-3 py-3"><div className="font-medium">{lead.leadName}</div><div className="text-xs text-neutral-500">{lead.email}</div></td>
                          <td className="px-3 py-3">{lead.campaignName}</td>
                          <td className="px-3 py-3">{lead.stepNumber ?? ""}</td>
                          <td className="px-3 py-3">{lead.category}</td>
                          <td className="max-w-[480px] px-3 py-3 text-red-700">{lead.reason}</td>
                        </tr>
                      ))}
                    </tbody>
                  </Table>
